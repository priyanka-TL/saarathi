...
def _remote_spec(key: str, *, company: str, finalize_path: str,
    ...
    remote = remote_dict(
        ...
        base_url=base_url or "https://remote.example.com",
    )
    return _adapter.validate_python({
        "schema_version": 1,
        "key": key,
        "name": f"Provider scope {key}",
        "description": "provider scope fixture",
...
def test_a_tenants_finalize_path_is_honoured_not_the_default_scopes(
    ...
    try:
        ...
        assert agent.spec.remote.options["finalize_path"] == "/api/end-story/"
        assert agent.spec.remote.options["company"] == "tenant-company"
...
def test_a_tenant_without_its_own_config_still_gets_the_default(
    ...
    try:
        ...
        assert agent.spec.remote.options["finalize_path"] == "/api/end-story/v2/"
        assert agent.spec.remote.options["company"] == "default-company"
    finally:
        session.close()
...
def test_two_scopes_get_two_different_providers(api_app, agent_with_scoped_mitra):
    """A provider carries the base URL, the timeouts and the credentials.
    Sharing one across scopes would send a tenant's story to another tenant's
    deployment -- the transport analogue of the shared-handler leak."""
    _key, agent_id = agent_with_scoped_mitra
    session = SessionLocal()
    try:
        orch = _orch(api_app, session)
        ...
        tenant_provider = orch.provider_for(tenant_agent)
        default_provider = orch.provider_for(default_agent)

        assert tenant_provider is not default_provider
        # Each endpoint comes from that scope's OWN config row. There is no
        # per-platform base-URL setting to compare against any more -- that is
        # the point: the default scope is just another row.
        assert tenant_provider._conn.base_url == "https://tenant-mitra.example.com"
        assert default_provider._conn.base_url == "https://remote.example.com"
    finally:
        session.close()


def test_the_same_scope_twice_reuses_one_provider(api_app, agent_with_scoped_mitra):
    ...
    try:
        ...
        first = orch.provider_for(orch.agent_for_session(_session_view(agent_id), _user(TENANT)))
        second = orch.provider_for(orch.agent_for_session(_session_view(agent_id), _user(TENANT)))
